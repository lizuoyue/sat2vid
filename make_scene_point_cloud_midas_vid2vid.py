...
if __name__ == '__main__':
    ...
    # files = sorted(glob.glob('london_vid2vid_test_template/*_07.png'))
    vec = get_pano_vec((512, 1024))
    for forder, file in tqdm.tqdm(list(enumerate(files))):
        ...
        # imgs = [np.array(Image.open(f'london_vid2vid_test_template/{fid}_%02d.png' % i)) for i in range(15)]
        deps = np.stack([decode_dep(item[:,:1024,:3]) for item in imgs])
        sems = np.stack([255-item[:,:1024,3] for item in imgs])
        rgbs = np.stack([item[:,1024:,:3] for item in imgs])
        ...
        os.system(f'mkdir -p london_vid2vid_xiaohu_test/seg_maps/stuttgart_00_{forder:03d}')
        os.system(f'mkdir -p london_vid2vid_xiaohu_test/images/stuttgart_00_{forder:03d}')
        os.system(f'mkdir -p london_vid2vid_xiaohu_test/unprojections/stuttgart_00_{forder:03d}')
        # os.system(f'mkdir -p london_vid2vid_xiaohu_template/seg_maps/stuttgart_00_{forder:03d}')
        # os.system(f'mkdir -p london_vid2vid_xiaohu_template/images/stuttgart_00_{forder:03d}')
        # os.system(f'mkdir -p london_vid2vid_xiaohu_template/unprojections/stuttgart_00_{forder:03d}')
        ...
        to_sel = [0, 0] + list(range(15))
        to_save = list(range(17))
        for i, j in zip(to_sel, to_save):
            ...
            Image.fromarray(sems[i]).save(f'london_vid2vid_xiaohu_test/seg_maps/stuttgart_00_{forder:03d}/stuttgart_00_000000_{j:06d}_leftImg8bit.png')
            Image.fromarray(rgbs[i]).save(f'london_vid2vid_xiaohu_test/images/stuttgart_00_{forder:03d}/stuttgart_00_000000_{j:06d}_leftImg8bit.jpg')
            # Image.fromarray(sems[i]).save(f'london_vid2vid_xiaohu_template/seg_maps/stuttgart_00_{forder:03d}/stuttgart_00_000000_{j:06d}_leftImg8bit.png')
            # Image.fromarray(rgbs[i]).save(f'london_vid2vid_xiaohu_template/images/stuttgart_00_{forder:03d}/stuttgart_00_000000_{j:06d}_leftImg8bit.jpg')

        # syn_rgbs, syn_sems, syn_idxs = [None] * 15, [None] * 15, [None] * 15
        syn_rgbs, syn_sems, syn_idxs = [],[],[]
        output = {}
        for i, j in zip(to_sel, to_save):
            delta = (i - 7) * 0.5
            to_move = ~(pc[:,-1].astype(np.uint8) == 10)
            ...
            pickle.dump(output, open(f'london_vid2vid_xiaohu_test/unprojections/stuttgart_00_{forder:03d}/stuttgart_00_000000_{j:06d}_leftImg8bit.pkl', 'wb'))
            # pickle.dump(output, open(f'london_vid2vid_xiaohu_template/unprojections/stuttgart_00_{forder:03d}/stuttgart_00_000000_{j:06d}_leftImg8bit.pkl', 'wb'))


            syn_rgbs.append(  Image.fromarray(syn[...,:3].astype(np.uint8)).resize((256,128),Image.LANCZOS)  )
            syn_sems.append(  Image.fromarray(syn[...,3].astype(np.uint8)).resize((256,128),Image.LANCZOS)  )
            syn_sems[-1].putpalette(palette)
            syn_idxs.append(  Image.fromarray(encode_index(syn[...,5]))  )
            pc[to_move,2] += delta
