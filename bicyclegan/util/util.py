...
def save_gif(list_image_numpy, image_path, aspect_ratio=1.0, save_img=False):
    """Save a numpy image to the disk
    Parameters:
        image_numpy (numpy array) -- input numpy array
        image_path (str)          -- the path of the image
    ...
    res = []
    for idx, image_numpy in enumerate(list_image_numpy):
        image_pil = Image.fromarray(image_numpy)
        h, w, _ = image_numpy.shape
        ...
        if aspect_ratio < 1.0:
            image_pil = image_pil.resize((int(h / aspect_ratio), w), Image.BICUBIC)
        res.append(image_pil)
        if save_img:
            image_pil.save(image_path.replace('.gif', '_%02d.png' % idx))
    res[0].save(image_path, save_all=True, append_images=res[1:], duration=150, loop=0)
