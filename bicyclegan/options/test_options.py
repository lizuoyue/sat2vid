...
class TestOptions(BaseOptions):
    def initialize(self, parser):
        ...
        parser.add_argument('--n_samples', type=int, default=0, help='#samples')
        parser.add_argument('--no_encode', action='store_true', help='do not produce encoded image')
        parser.add_argument('--per_frame_test', action='store_true', help='test frame by frame')
        parser.add_argument('--sync', action='store_true', help='use the same latent code for different input images')
        parser.add_argument('--save_img', action='store_true', help='use the same latent code for different input images')
